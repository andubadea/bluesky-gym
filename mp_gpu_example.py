...
Also note that multiprocessing doesn't necessarily result in faster training. 
It is highly dependent on the environment and algorithm combination. If the 
algorithm is able to train over a batch of observations, multiprocessing should 
...
if device =="mps":
    ...
    SubprocVecEnv = sb3.common.vec_env.SubprocVecEnv
...
if __name__ == "__main__":
    ...
    if TRAIN:
        ...
        model.save(f"models/{env_name}/{env_name}_{str(algorithm.__name__)}/model_mp_gpu")
        del model
    env.close()
    
    # Test the trained model
    model = algorithm.load(f"models/{env_name}/{env_name}_{str(algorithm.__name__)}/model_mp_gpu", env=env)
