...
uses the GPU for training. By default, stable-baselines3 should select the GPU
as a device, but only if a CUDA-capable GPU is present. This script also makes it
possible to run the training on MPS GPUs (Apple Silicon chipsets).
